...
def load_line_data(script_dir, line_num):
    """Load model data from a specific line directory."""
    results_dir = os.path.join(script_dir, 'results', f'29_line_{line_num}', 'Results')
    
    if not os.path.exists(results_dir):
        raise ValueError(f"Results directory not found: {results_dir}")
    
    # Find the most recent generation folder (e.g., 29_gen30ind14)
    ...
    print(f"Line {line_num}: Using generation folder: {gen_folder}")
    ...
    print(f"Line {line_num}: Using timestamp folder: {timestamp_folder}")
    
    # Find the .gdat file
    gdat_pattern = os.path.join(timestamp_folder_path, f"{gen_folder}_29.gdat")
    ...
    print(f"Line {line_num}: Using gdat file: {os.path.basename(gdat_file)}")
    ...
    return model_time, model_H_weekly

def create_plot(script_dir, exp_dates, exp_H_weekly, model_data_list, num_trajectories, output_filename):
    """Create a plot with specified number of trajectories."""
    ...
    # Plot model trajectories (up to num_trajectories)
    for i in range(min(num_trajectories, len(model_data_list))):
        model_time, model_H_weekly = model_data_list[i]
    ...
    #ax.set_title(f'Arizona Flu Hospitalization Data - MLE Fit ({num_trajectories} Trajectory{"ies" if num_trajectories > 1 else ""})', 
    ...
    all_model_dates = []
    for model_time, model_H_weekly in model_data_list[:num_trajectories]:
        model_dates = [start_date + timedelta(weeks=float(week)) for week in model_time]
        all_model_dates.extend(model_dates)
    
    all_dates = exp_dates + all_model_dates
    max_y = max(max(exp_H_weekly), 
                max([max(model_H_weekly) for _, model_H_weekly in model_data_list[:num_trajectories]]))
...
try:
    ...
    print(f"Hospitalization data: {len(exp_H_weekly)} points")
    
    # Load data from all 4 lines
    model_data_list = []
    for line_num in range(1, 5):
        try:
            model_time, model_H_weekly = load_line_data(script_dir, line_num)
            model_data_list.append((model_time, model_H_weekly))
            print(f"Line {line_num} data: {len(model_H_weekly)} points")
        except Exception as e:
            print(f"Warning: Could not load data from line {line_num}: {e}", file=sys.stderr)
    
    if not model_data_list:
        raise ValueError("No model data could be loaded from any line")
    
    # Create 4 plots with increasing numbers of trajectories
    for num_trajectories in range(1, 5):
        if num_trajectories <= len(model_data_list):
            output_filename = f'az_MLE_plot_{num_trajectories}traj.png'
            create_plot(script_dir, exp_dates, exp_H_weekly, model_data_list, 
                       num_trajectories, output_filename)
